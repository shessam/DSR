...
for forward_data in forward_data_lst:
    ...
    if _run_forwarding_in_subprocesses(config):
        ...
        for info_file in info_files:
            if not (os.path.exists(info_file)):
                sys.stderr.write(
                    "ERROR: File %s does not exist. Forwarding did not succeeded.\nSee %s \n" % (info_file, log_file)
                )
                sys.exit(0)
...
# valid_data_lst = config["data_use"]["valid_with"].split(",")
# valid_outs = config["valid"]["valid_out"].split(",")
# valid_dec_outs = list(map(strtobool, config["valid"]["require_decoding"].split(",")))
# # print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 540")

# for data in valid_data_lst:
#     for k in range(len(valid_outs)):
#         if valid_dec_outs[k]:

#             print("Decoding %s output %s" % (data, valid_outs[k]))

#             info_file = out_folder + "/exp_files/decoding_" + data + "_" + valid_outs[k] + ".info"

#             # create decode config file
#             config_dec_file = out_folder + "/decoding_" + data + "_" + valid_outs[k] + ".conf"
...
#             files_dec = out_folder + "/exp_files/valid_" + data + "_ep*_ck*_" + valid_outs[k] + "_to_decode.ark"
#             out_dec_folder = out_folder + "/decode_" + data + "_" + valid_outs[k]
