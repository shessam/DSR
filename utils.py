...
def parse_fea_field(fea):
    ...
    for fea_lst in fea_lsts:
        ...
        if not (os.path.isfile(fea_lst)):
            ...
            ####### DIFFERENT SENTS #####
            snts = sorted([line.rstrip("\n").split(" ")[0].split('_')[0] for line in open(fea_lst)])
            snt_lst.append(snts)
            # Check if all the sentences are present in all the list files
            # if cnt > 0:
            #     print('snt_lst[cnt - 1]: ', len(snt_lst[cnt - 1]))
            #     print('snt_lst[cnt]: ', len(snt_lst[cnt]))
            #     print('type:', type(snt_lst[cnt]))
...
def dump_epoch_results(
        res_file_path, ep, tr_data_lst, tr_loss_tot, tr_error_tot, tot_time, valid_data_lst, valid_peformance_dict, lr,
        N_ep
):
    #
