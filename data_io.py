...
def load_dataset(
    ...
    def _read_features_and_labels_with_kaldi(fea_scp, fea_opts, fea_only, lab_folder, lab_opts, output_folder):
        with open("/home/hessam/kaldi/egs/ami/s5b/removing_overlapped/overlapped.txt", 'r') as FILE0:
            a = FILE0.readlines()
        ...
        if _input_is_feature_file(fea_scp):
            ...
            kaldi_bin = "wav-copy"
            read_function = read_vec_flt_ark
        # print("ark:" + kaldi_bin + " scp:" + fea_scp + " ark:- |" + fea_opts)
        # print('read function:   ', read_function("ark:" + kaldi_bin + " scp:" + fea_scp + " ark:- |" + fea_opts, output_folder))
        fea = {
            k: m
            for k, m in read_function("ark:" + kaldi_bin + " scp:" + fea_scp + " ark:- |" + fea_opts, output_folder)
        }
        # print('len(fea)**: ', len(fea))
        ### removing _LA1
        # fea_fixed = {}
        # fea_fixed = fea
        # for key in fea.keys():
        #     fea_fixed[key.split('_')[0]] = fea[key]
        if not fea_only:
            # print('not fea_only')
            # print('text lab: ', "gunzip -c " + lab_folder + "/ali*.gz | " + lab_opts + " " + lab_folder + "/final.mdl ark:- ark:-|")
            lab = {
                k: v
                for k, v in read_vec_int_ark(
                    "gunzip -c " + lab_folder + "/ali*.gz | " + lab_opts + " " + lab_folder + "/final.mdl ark:- ark:-|",
            ...
            count = 0
            lab_fixed = {
                            # "".join("".join("".join("".join("".join(key.split('_H00')).split('_SDM')).split('_H01')).split('_H02')).split('_H03')):v
                            key:v
                            for key, v in zip(lab.keys(), lab.values())
                        }
            fea_fixed = {
                            # "".join(key.split('_SDM')):v
                            key:v
                            for key, v in zip(fea.keys(), fea.values())
                        }            
            # print("***************************START*****************************")
            # for key, v in zip(lab_fixed.keys(), lab_fixed.values()):
            #     print(key)  
            #     count += 1
            #     if count > 10:
            #         break
            # print("HHHHHHHHHHHHHHHHHHHHHHHH")
            # for key, v in zip(fea_fixed.keys(), fea_fixed.values()):
            #     print(key)  
            #     count += 1
            #     if count > 20:
            #         break
            ...
            # for key in lab.keys():
            #     lab_fixed[key.split('_')[0]] = lab[key]

            # for count, i, j in zip(range(len(lab_fixed.keys())), sorted(lab_fixed.keys()), sorted(fea_fixed.keys())):
            #     print("lab, fea [{}]".format(count))
            #     print(i, j)
            #     print("---------------")
            #     if count > 10:
            #         break
            # print('lab 12345: ', lab)
            fea_fixed = {
                k: v for k, v in fea_fixed.items() if k in lab_fixed
            }  # This way I remove all the features without an aligment (see log file in alidir "Did not Succeded")
            # fea_fixed = {
            #     k: v for k, v in fea_fixed.items() if not k in a
            # }
            # lab_fixed = {
            #     k: v for k, v in lab_fixed.items() if not k in a
            # }
            ### ****** IHM Alignment lab fix ****** ###
            # lab_fixed = {
            #     k: v for k, v in lab_fixed.items() if not k.split('_')[1] in b
            # }
            # print(len(lab_fixed))
            # for key in lab_fixed.keys():
            #     print(lab_fixed[key.split('_')[1]])
            # print('len(fea_fix)**: ', len(fea_fixed))
            # print('len(lab_fix)**: ', len(lab_fixed))

        return fea_fixed, lab_fixed
    ...
    def _concatenate_features_and_labels(fea_conc, lab_conc):
        def _sort_chunks_by_length(fea_conc, lab_conc):
            # print('ln f_c', len(fea_conc))
            # print('ln l_c', len(lab_conc))
        ...
        # print('fea sort_chunk', len(fea_conc), fea_conc[0].shape)
        # print('lab sort_chunk', len(lab_conc), lab_conc[0].shape)
        
        fea_conc = np.array(fea_conc)
        lab_conc = np.array(lab_conc)

        for p in range(len(lab_conc)):
            if fea_conc[p].shape[0] > lab_conc[p].shape[0]:
                fea_conc[p] = fea_conc[p][:lab_conc[p].shape[0]]
            if fea_conc[p].shape[0] < lab_conc[p].shape[0]:
                # print(type(lab_conc[p]), type(fea_conc[p].shape[0]))
                lab_conc[p] = lab_conc[p][:fea_conc[p].shape[0]]


        fea_conc = list(fea_conc)
        lab_conc = list(lab_conc)
            ...
            # print('P: ', p)
            # print('fea indoor', np.shape(fea_conc[p]))
            # print('lab indoor', np.shape(lab_conc[p]))
            # else:
            #     print('EQ')
        ...
        # print('fea conc sort_chunk', fea_conc.shape)
        # print('lab conc sort_chunk', lab_conc.shape)
    ...
    # print('len(f_chunk)', len(fea_chunks))
    # print('len(l_chunks)', len(lab_chunks))
...
def load_chunk(
    ...
    # print('aalliiiiii')
    # print(fea_scp, fea_opts, lab_folder, lab_opts, left, right, max_sequence_length, output_folder, fea_only, sep='\n')
    # print('bbbbbbbb')
    # open the file
    ...
    # print(data_set)
    # print('data lab len:', len(data_lab))
    # print(data_lab)
...
def read_lab_fea(cfg_file, fea_only, shared_list, output_folder):
    ...
    for fea in fea_dict.keys():
        ...
        for lab in lab_dict.keys():
            ...
            # print(fea_scp, fea_opts, lab_folder, lab_opts, cw_left, cw_right, max_seq_length, output_folder, fea_only)
...
def read_vec_int_ark(file_or_fd, output_folder):
  ...
  """
    # print('file_or_fd 1234: ', file_or_fd)
