...
class QLSTM(nn.Module):
    ...
    def __init__(self, options, inp_dim):
        ...
        # self.enc = R2HQDNN(1024, 'tanh', True, inp_dim, 4 * inp_dim)
    ...
    def forward(self, x):
        # print('Before Encoder QNN', x.shape)
        # x = self.enc(x)
        # print('After Encoder QNN', x.shape)
        # print(x)
        for i in range(self.N_lstm_lay):
            ...
            x = h
        # print('end before reshape QNN', x.shape)

        # x = x.reshape(x.shape[2])
        # print('end after QNN', x.shape)
